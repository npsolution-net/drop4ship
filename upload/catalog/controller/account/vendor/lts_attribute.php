...
class ControllerAccountVendorLtsAttribute extends Controller {
  ...
  public function add() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      ...
      $this->response->redirect($this->url->link('account/vendor/lts_attribute', $url));
    }
  ...
  protected function getList() {
    ...
    $data['breadcrumbs'][] = array(
        ...
        'href' => $this->url->link('account/vendor/lts_attribute', $url)
    );

    $data['add'] = $this->url->link('account/vendor/lts_attribute/add');
    $data['delete'] = $this->url->link('account/vendor/lts_attribute/delete');
  ...
  protected function getForm() {
   ...
   if(!$vendor_info) {
    ...
    $data['attribute_groups'] = $this->model_account_vendor_lts_attribute_group->getAttributeGroups();
