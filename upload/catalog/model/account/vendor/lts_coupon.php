<?php
class ModelAccountVendorLtsCoupon extends Model {
	public function addCoupon($vendor_id, $data) {
		...
		$this->db->query("INSERT INTO ". DB_PREFIX ."lts_coupon SET coupon_id='". (int)$coupon_id ."', vendor_id='". (int)$vendor_id ."', code='". $this->db->escape($data['code']) ."'");
	...
	public function editCoupon($coupon_id, $vendor_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "lts_coupon SET code = '" . $this->db->escape($data['code']) . "' WHERE coupon_id = '" . (int)$coupon_id . "' AND vendor_id='". (int)$vendor_id ."'");
	...
	public function deleteCoupon($coupon_id, $vendor_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "lts_coupon WHERE coupon_id = '" . (int)$coupon_id . "' AND vendor_id='". (int)$vendor_id ."'");
	...
	public function getCoupons($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "coupon c JOIN ". DB_PREFIX ."lts_coupon lc ON(c.coupon_id = lc.vendor_id) ";

		$sql .= "WHERE lc.vendor_id = '". (int)$data['vendor_id'] ."'";
	...
	public function getTotalCoupons($data = array()) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "lts_coupon WHERE vendor_id='". $data['vendor_id'] ."'");
