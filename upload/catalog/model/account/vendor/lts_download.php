<?php
class ModelAccountVendorLtsDownload extends Model {
	public function addDownload($vendor_id, $data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "lts_download SET download_id = '". (int)$download_id ."', vendor_id = '" . (int)$vendor_id . "'");
	...
	public function editDownload($download_id, $vendor_id, $data) {
	...
	public function deleteDownload($download_id, $vendor_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "download WHERE download_id = '" . (int)$download_id . "'");

		$this->db->query("DELETE FROM " . DB_PREFIX . "lts_download WHERE download_id = '" . (int)$download_id . "' AND vendor_id = '".  (int)$vendor_id ."'");
	...
	public function getDownloads($data = array()) {
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "download d LEFT JOIN " . DB_PREFIX . "download_description dd ON (d.download_id = dd.download_id)  RIGHT JOIN `" . DB_PREFIX . "lts_download` ld ON (d.download_id = ld.download_id) WHERE dd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ld.vendor_id='". (int)$data['vendor_id'] ."'";
	...
	public function getTotalDownloads($data = array()) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "lts_download WHERE vendor_id='". (int)$data['vendor_id'] ."'");
