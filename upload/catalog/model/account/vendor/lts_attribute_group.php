<?php
class ModelAccountVendorLtsAttributeGroup extends Model {
	public function addAttributeGroup($vendor_id, $data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "attribute_group SET sort_order = '" . (int)$data['sort_order'] . "'");

		$attribute_group_id = $this->db->getLastId();

		$this->db->query("INSERT INTO " . DB_PREFIX . "lts_attribute_group SET attribute_group_id = '" . (int)$attribute_group_id . "', vendor_id = '" . (int)$vendor_id . "'");
	...
	public function editAttributeGroup($attribute_group_id, $vendor_id, $data) {
	...
	public function deleteAttributeGroup($attribute_group_id, $vendor_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "attribute_group WHERE attribute_group_id = '" . (int)$attribute_group_id . "'");

		$this->db->query("DELETE FROM " . DB_PREFIX . "lts_attribute_group WHERE attribute_group_id = '" . (int)$attribute_group_id . "' AND vendor_id = '" . (int)$vendor_id . "'");
	...
	public function getAttributeGroups($data = array()) {
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "lts_attribute_group lag LEFT JOIN " . DB_PREFIX . "attribute_group ag ON (lag.attribute_group_id = ag.attribute_group_id) LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON(ag.attribute_group_id = agd.attribute_group_id) WHERE agd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND lag.vendor_id='" . (int)$data['vendor_id'] . "'";
	...
	public function getTotalAttributeGroups($data = array()) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "lts_attribute_group WHERE vendor_id = '" . (int)$data['vendor_id'] . "'");
