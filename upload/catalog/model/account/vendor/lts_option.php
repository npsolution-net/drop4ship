<?php
class ModelAccountVendorLtsOption extends Model {
	public function addOption($vendor_id, $data) {

		$this->db->query("INSERT INTO `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', sort_order = '" . (int)$data['sort_order'] . "'");

		$option_id = $this->db->getLastId();

		$this->db->query("INSERT INTO `" . DB_PREFIX . "lts_option` SET option_id = '" . (int)$option_id . "', vendor_id = '" . (int)$vendor_id . "'");
	...
	public function editOption($option_id, $vendor_id, $data) {
	...
	public function deleteOption($option_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "lts_option` WHERE option_id = '" . (int)$option_id . "' AND vendor_id = '" . (int)$this->customer->getId() . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "option_description WHERE option_id = '" . (int)$option_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "option_value WHERE option_id = '" . (int)$option_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "option_value_description WHERE option_id = '" . (int)$option_id . "'");
	}
	...
	public function getOptions($data = array()) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "lts_option` lo LEFT JOIN " . DB_PREFIX . "option o ON (lo.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON(lo.option_id = od.option_id) WHERE od.language_id = '" . (int)$this->config->get('config_language_id') . "' AND lo.vendor_id = '" . (int)$data['vendor_id'] . "'";
	...
	public function getTotalOptions($data = array()) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "lts_option` WHERE  vendor_id='". (int)$data['vendor_id'] ."'");
