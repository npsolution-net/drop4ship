<?php
class ModelAccountVendorLtsFilter extends Model {
	public function addFilter($vendor_id, $data) {

		$this->db->query("INSERT INTO `" . DB_PREFIX . "filter_group` SET sort_order = '" . (int)$data['sort_order'] . "'");

		$filter_group_id = $this->db->getLastId();

		$this->db->query("INSERT INTO `" . DB_PREFIX . "lts_filter_group` SET filter_group_id = '" . (int)$filter_group_id . "', vendor_id = '" . (int)$vendor_id . "'");
	...
	public function editFilter($filter_group_id, $vendor_id, $data) {
	...
	public function deleteFilter($filter_group_id, $vendor_id) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter_group` WHERE filter_group_id = '" . (int)$filter_group_id . "'");

		$this->db->query("DELETE FROM `" . DB_PREFIX . "lts_filter_group` WHERE filter_group_id = '" . (int)$filter_group_id . "' AND vendor_id = '". (int)$vendor_id ."'");
	...
	public function getFilterGroups($data = array()) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "lts_filter_group` lfg LEFT JOIN " . DB_PREFIX . "filter_group fg ON(lfg.filter_group_id = fg.filter_group_id) LEFT JOIN " . DB_PREFIX . "filter_group_description fgd ON(lfg.filter_group_id = fgd.filter_group_id) WHERE fgd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND vendor_id = '" . (int)$data['vendor_id'] . "' ";
	...
	public function getTotalFilterGroups($data = array()) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "lts_filter_group` WHERE vendor_id='". (int)$data['vendor_id'] ."'");
